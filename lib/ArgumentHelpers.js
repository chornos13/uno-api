...
//ex: a or [a] or [a, b, c]
function useSingleArray(data, cb, forceDataToArray) {
    if (forceDataToArray === void 0) { forceDataToArray = false; }
    if (data) {
        if (Array.isArray(data)) {
            for (var i = 0; i < data.length; i++) {
                cb(data[i]);
...
//ex: a or [a] or [a, b, c]
function getSingleArray(data, forceDataToArray) {
    if (forceDataToArray === void 0) { forceDataToArray = false; }
    if (data) {
        if (Array.isArray(data)) {
            return data;
...
//ex: a or {a} or [a]
function useSingleArrayAndObject(data, cb, cbObject, forceDataToArray) {
    if (forceDataToArray === void 0) { forceDataToArray = false; }
    if (data) {
        if (isPureObject(data)) {
            cbObject(data);
...
module.exports = {
    useMultiArray: useMultiArray,
    useObject: useObject,
    useSingleArray: useSingleArray,
    getSingleArray: getSingleArray,
    useMultiArrayAndObject: useMultiArrayAndObject
};
